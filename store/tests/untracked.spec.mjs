...
describe('untracked with signals', () => {
    /** @type {ReturnType<typeof scope>} */
    let testScope;
    ...
    it('returns the value from callback', () => {
        ...
        const result = untracked(() => value() * 2);
    ...
    it('computed accessed inside untracked does not create dependency', async () => {
        ...
        // Changing signal should recompute the computed but NOT re-run the effect
        value.set(5);
        // Access the computed to trigger recomputation
        expect(doubled()).toBe(10);
        expect(computeRuns).toBe(2);
